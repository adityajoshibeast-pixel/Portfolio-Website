...
export default function ChatWidget() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", content: "Hi! Ask me anything about Aditya's projects or skills." },
  ]);
  ...
  return (
    <>
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 flex h-[420px] w-[340px] flex-col overflow-hidden rounded-2xl border border-surface-2 bg-surface shadow-2xl">
          <div className="flex items-center justify-between border-b border-surface-2 bg-surface-2/50 px-4 py-3">
            <span className="font-mono text-sm text-accent">chat.ask()</span>
            <button
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-accent text-bg shadow-lg transition-transform hover:scale-105"
      >
        {isOpen ? "✕" : "💬"}
      </button>
    </>
